...
def add_columns_by_id(
    df: pl.LazyFrame, other_df: pl.LazyFrame, on: str, columns: list[str]
) -> pl.LazyFrame:
    """Joins the two dataframes by ``on`` and adds columns to the original dataframe."""

    other_df = other_df.select(on, *columns)
    return df.join(other_df, on=on, how="left")


add_discharge_time_by_hadm_id = partial(add_time_by_id, on="hadm_id", time_column_name="dischtime")
add_discharge_and_admit_time_by_hadm_id = partial(
    add_columns_by_id, on="hadm_id", columns=["dischtime", "admittime"]
)
...
def prepare_mimic_cxr_metadata(df: pl.LazyFrame) -> pl.LazyFrame:
    """Prepares the MIMIC-CXR metadata for MEDS processing.

    Args:
        df: The MIMIC-CXR metadata.

    Returns:
        The processed MIMIC-CXR metadata.
    """
    return df.with_columns(
        time=pl.col("StudyDate")
        .cast(str)
        ...
        .cast(str),
        dicom_id=pl.col("dicom_id").hash().cast(str).str.slice(0, 14).cast(int),
    )
