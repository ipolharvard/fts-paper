...
from ..utils import apply_vocab_to_multitoken_codes, process_icd_cm_10, unify_code_names
    ...
    def place_death_before_dc_if_same_time(df: pl.DataFrame) -> pl.DataFrame:
        ...
        return (
            df.with_columns(
                ...
                # TODO: this time shift is unnecessary
                .then(pl.col("time").dt.combine(time=time(23, 59, 59))).otherwise("time")
            )
...
class DiagnosesData:
    ...
    def process_icd10(df: pl.DataFrame, vocab: list[str] | None = None) -> pl.DataFrame:
        return process_icd_cm_10(df, icd_col=pl.col("text_value"), vocab=vocab)
...
class ProcedureData:
    ...
    def process_icd10(icd10_df: pl.DataFrame, vocab: list[str] | None = None) -> pl.DataFrame:
        df = icd10_df.with_columns(
            ...
            code=pl.concat_list(pl.lit("ICD//PCS//") + pl.col("code").struct[i] for i in range(7))
        )
...
class LabData:
    ...
    @staticmethod
    @MatchAndRevise(prefix="LAB//", needs_counts=True)
    def make_quantiles(df: pl.DataFrame, counts: dict[str, int] | None = None) -> pl.DataFrame:
        if counts is not None:
            df = df.filter(pl.col("code").is_in(list(counts.keys())[:200]))
        return df.with_columns(
            code=pl.concat_list(
                "code",
                pl.lit("LAB//Q//") + pl.col("code").str.slice(len("LAB//")),
            )
        ).explode("code")
