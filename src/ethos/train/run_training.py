...
def main(cfg: DictConfig):
    ...
    def get_epoch(i: int) -> float:
        return i * tokens_per_iter / len(train_dataset)
    ...
    iter_num, best_val_loss, optimizer_state, wandb_path = 0, 1e9, None, None
    if cfg.resume:
    ...
    while True:
        ...
        if int(epoch := get_epoch(iter_num)) != int(get_epoch(iter_num - 1)) and master_process:
            ...
            val_loss_deque.append(losses["loss/val"])

            if (val_loss := sum(val_loss_deque) / len(val_loss_deque)) < best_val_loss:
